...
Use your Read tool to read the file `~/.claude/commands/klient-engine/summarize.md`. Display the FULL contents of the file to the user so they can see it.
...
Create the file `~/.claude/commands/klient-engine/skill-builder.md` with this content:
...
# /klient-engine:skill-builder — Build a New Skill
...
```
  ...
  ║                                               ║
  ║  Type /klient-engine:skill-builder anytime to          ║
...
- The gift skill-builder file must be saved to `~/.claude/commands/klient-engine/skill-builder.md` — inside the klient-engine namespace so it's clearly from the course
