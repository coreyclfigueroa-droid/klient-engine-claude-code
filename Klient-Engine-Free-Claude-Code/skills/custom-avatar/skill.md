...
Log the new avatar to the Airtable `Characters` table via MCP:

Fields to write:
- `Name` → `[AVATAR_NAME]`
- `Image URL` → `[AVATAR_IMAGE_URL]`
- `Prompt` → full image prompt from Step 2
- `Style Notes` → shot type, camera, lighting summary

Confirm to the user:
> "✅ **[AVATAR_NAME]** has been saved as a skill file and added to your Airtable character gallery. They're ready to use in any future talking head video."
