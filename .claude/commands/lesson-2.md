...
Spin up a local dev server and open the website in the browser:

bash   echo "🌐 Spinning up your website..." && \
   npx --yes serve ./my-website --listen tcp://localhost:3000 &
   SERVER_PID=$! && \
   ...
   echo "✅ Your website is live at → http://localhost:3000"
...
Say:
That's YOUR business. YOUR name. YOUR colors and vibe — live in the browser right now.
...
I'm about to run the content engine RIGHT NOW — inline, right here — so you can watch it work in real time.
...
Now execute the following automatically WITHOUT waiting:

Read the user's CLAUDE.md:
bash   cat ./CLAUDE.md

Read the viral hooks skill:
bash   cat ./skills/viral-hooks-skill/SKILL.md

Read the hooks library:
bash   cat ./skills/viral-hooks-skill/references/hooks-library.md

Read the 7-factor framework:
bash   cat ./skills/viral-hooks-skill/references/seven-factors-framework.md

Using everything you just read — the user's CLAUDE.md, the SKILL.md framework, the hooks library, and the 7-factor system — generate a FULL WEEK of content (7 days). For each day include: Hook (Verbal), Hook (Written), Hook (Visual), TikTok caption, Instagram Reels caption + hashtags, YouTube Shorts caption, Angle, Format. Every piece must be fully personalized to their business — no placeholders, no "insert X here." Ready to film or post immediately.

After generating all 7 days, immediately say:
YOOO!!! 🎉🎉🎉
You see that?! A full week of content. Hooks pulled from the library. All 7 factors applied. Captions written in YOUR voice. Formatted for every platform. Ready to film or post RIGHT NOW.
That's not ChatGPT giving you "5 content ideas." That's a content SYSTEM. And now you have a slash command — /content-engine — that runs this exact thing whenever you want. Every week. Forever. One command. 🔥
That skill folder at ./skills/viral-hooks-skill/? It's yours. Edit the hooks, tweak the framework, add your own angles. YOUR files. 📄
