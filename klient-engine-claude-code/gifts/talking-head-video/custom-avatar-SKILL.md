...
Read `AIRTABLE_BASE_ID` from the user's `~/.claude/settings.json` env settings. This is their own Klient Engine base set up in Lesson 3. Log to their base — NOT the shared gallery base.

Search the AI Avatars table in the user's base for an existing record with this Name. If found, update it. If not, create a new record.

Fields to write:
- `Name` → `[AVATAR_NAME]`
- `Image URL` → `[AVATAR_IMAGE_URL]`
- `Full Prompt` → full image prompt from Step 2
- `Shot Type` → shot type chosen
- `Camera` → camera and focal length
- `Lighting` → lighting summary
- `Character Description` → brief description of appearance and vibe

Confirm to the user:
> "✅ **[AVATAR_NAME]** has been saved as a skill file and logged to your Airtable. They're ready to use in any future talking head video."

Then return to the Talking Head Video skill at Step 4 — Avatar Selection — and continue with `[AVATAR_NAME]` already loaded.
