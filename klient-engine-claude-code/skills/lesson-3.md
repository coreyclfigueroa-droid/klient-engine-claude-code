...
This is what lets Claude not only read and write your data, but also create new tables and set up your project tracker automatically.
...
**Beautiful. Now watch this.** 😏

I'm about to do something cool.

Instead of making you go dig through Airtable to find some Base ID and copy-paste it manually...

**I'm just gonna create your entire project base for you. Right now. From here.**
...
This script does everything in sequence. Run each step and handle errors inline.

**Step A — Validate the PAT and get the workspace:**

```bash
# Get the PAT from settings.json
AIRTABLE_PAT=$(python3 -c "
import json, os
with open(os.path.expanduser('~/.claude/settings.json')) as f:
    settings = json.load(f)
print(settings.get('env', {}).get('AIRTABLE_API_KEY', ''))
")
...
Extract the first workspace ID from the response. If the request fails (non-200), stop and tell the user their token was rejected — they need to check their scopes.

**Step B — Check for an existing Klient Engine base:**
...
Parse the response. If any base name contains "klient" AND "engine" (case-insensitive), use that base's ID and skip creation. Otherwise, proceed to Step C.

**Step C — Create the Klient Engine base with the Projects table:**
...
{
  "name": "Klient Engine",
  "workspaceId": "<workspace_id from Step A>",
  "tables": [
    {
      "name": "Projects",
      "fields": [
...
Extract the base ID from the response.

**Step D — Save the Base ID to settings.json:**
...
if 'env' not in settings:
    settings['env'] = {}
settings['env']['AIRTABLE_BASE_ID'] = '<BASE_ID from Step C>'
...
**Step E — Write one sample record to prove the connection:**
...
If the PAT is rejected or the base creation fails, say:
...
You should see a brand new base called **"Klient Engine"** with a **Projects** table inside it.

There's already a record in there — Claude just wrote it.

**You didn't create a base. You didn't make a table. You didn't copy a single ID.**
...
1. **Token scopes** — when you created your token, you needed to select **data.records:read**, **data.records:write**, **schema.bases:read**, AND **schema.bases:write**. If you missed one, create a new token with all four.
...
**Step A:** Call fal.ai to generate an image.

```
POST https://queue.fal.run/fal-ai/nano-banana-pro
...
Note: fal.ai queue endpoints may return a request_id for polling. If so, poll the status endpoint until complete, then retrieve the result. Extract the image URL from the response.

**Step B:** Write a record to the **"Projects"** table in the Klient Engine base with the result:
...

{
  "records": [
    {
      "fields": {
        "Ad Name": "🎨 fal.ai Connection Test",
        "Product Name": "Black Water Bottle",
        "Image Prompt": "Model: fal-ai/nano-banana-pro\n\nPrompt: professional product photography of a sleek black water bottle on a marble surface, studio lighting, minimalist, commercial quality",
        "Image Generator": "fal.ai",
        "Image Status": "Generated",
        "Generated Image 1": "<insert generated image URL here>"
      }
    }
  ]
}
...
After both calls succeed, output:
...
You should see a new record — **"fal.ai Connection Test"** — with the image URL, the prompt, the model used, and the status all logged.
