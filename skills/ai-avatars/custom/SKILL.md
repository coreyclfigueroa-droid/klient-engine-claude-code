---
name: custom
description: Build a custom AI avatar from scratch — intake, UGC-style prompt, fal.ai generation, skill file save, and Airtable logging.
---
...
The prompt style that works is hyper-specific and mundane. Study this example:
...
- Tight or close-up framing — subject must fill 50-60% of frame max, never 80%+
...
First pass (no reference image yet) — use base model:
```json
{
...
If user provides a reference image — upload it first with `mcp__fal__upload`, then use edit endpoint:
```json
{
...
Poll with `mcp__fal__status` until COMPLETED, then fetch with `mcp__fal__result`. Retrieve `images[0].url`. Store as `[AVATAR_IMAGE_URL]`.
...
## STEP 5 — SAVE THE AVATAR SKILL FILE

Auto-save. Do not ask permission — just save and confirm.

Create a new file at:
`/skills/ai-avatars/[avatar-name-lowercase-hyphenated]/SKILL.md`

```markdown
---
name: [lowercase-hyphenated-name]
description: AI avatar for [AVATAR_NAME] — [brief one-line description of appearance]
---

# Avatar: [AVATAR_NAME]
# Path: /skills/ai-avatars/[avatar-name]/SKILL.md
...
The **AI Avatars** table already exists. Do NOT try to create it. Log the record using curl:

```bash
curl -s -X POST "https://api.airtable.com/v0/[AIRTABLE_BASE_ID]/AI%20Avatars" \
...
**CRITICAL — always write BOTH fields:**
- `Image URL` → plain text URL string (for display and reference)
- `Image` → attachment array `[{"url": "..."}]` (so the actual image shows up in Airtable)

If a record with this name already exists — use PATCH to update it instead of POST.
