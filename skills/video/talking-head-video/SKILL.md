...
STEP 1 — AVATAR + ENVIRONMENT SELECTION (COMBINED)
...
3. Append: 9:16 vertical composition. Medium shot — subject fills 50-60% of frame. Never tight, never close-up.
4. Reference image for character consistency: [AVATAR_REFERENCE_URL]
Store full prompt as [IMAGE_PROMPT].

STEP 3 — GENERATE IMAGE
Tell the user: "Image generation cost: **$0.15** (fal-ai/nano-banana-pro)"

Call fal.ai via MCP (`mcp__fal__generate`). Never use curl with raw API keys.

If [AVATAR_REFERENCE_URL] exists (it will for any saved avatar):
- Use `fal-ai/nano-banana-pro/edit` with `image_urls: [[AVATAR_REFERENCE_URL]]`

If no reference image exists (brand new avatar, first pass only):
- Use `fal-ai/nano-banana-pro` base model
...
Poll with `mcp__fal__status` until COMPLETED, then fetch with `mcp__fal__result`.
...
Field | Value
Ad Name | [AVATAR_NAME] — [ENVIRONMENT_NAME] — [today's date]
Product Name | (updated later if video is generated)
...
**4C — AI Environments table (ALWAYS)**
...
Show the user this cost table so they can pick their combo:

"Here's what the video costs by duration and audio mode:
...
Image already generated: +$0.15 on top of any video cost.
Any duration works — not limited to the examples above.

How long do you want the video (seconds)? And audio ON, OFF, or ON + Voice Control?"

Wait for both answers. Store as [VIDEO_DURATION] and [AUDIO_MODE].
...
Calculate:
- [VIDEO_COST] = [VIDEO_DURATION] × rate. Round to 2 decimal places.
- [TOTAL_COST] = $0.15 (image) + [VIDEO_COST]. Round to 2 decimal places.

Confirm to the user:

"Got it — here's your cost:
- Image: $0.15 (already generated)
- Video: [VIDEO_DURATION]s × $[rate]/sec (audio [AUDIO_MODE]) = $[VIDEO_COST]
- **Total: $[TOTAL_COST]**

Type YES to generate."

Wait for YES before proceeding to Step 9.

STEP 9 — BUILD VIDEO PROMPT
...
STEP 11 — UPDATE AIRTABLE RECORD
